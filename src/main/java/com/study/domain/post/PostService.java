...
public class PostService {
	 ...
	 * @return Generated PK
	 */
	// 스프링에서 제공하는 트랜잭션 처리방법, "선언적 트랜잭션으로 불림 호출된 메서드에 해당 어노테이션이 선언되어 있으면 메서드의 실행과 동시에 트랜잭션 시작,
	...
	public Long savePost(final PostRequest params) { // savePost() 게시글 생성, INSERT 완료시 생성된 게시글 id 리턴
		...
		return params.getId();  // 생성된 게시글 ID가 null이 되지 않기 위해서는 PK값을 객체에 담아줘야한다 이것을 MyBatis의 "useGeneratedKeys" 기능 이용 --> PostMapper.xml 이동
	}
